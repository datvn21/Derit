    ...
    try {
      ...
      const sessionsWithCount = sessions.map((session) => {
        session.submittedCount =
          submissionCountMap[session._id.toString()] || 0;
        return session;
      });
...
examSessionRouter.get("/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    if (user.role === "lecturer") {
      ...
      session = await ExamSessionModel.findById(req.params.id).populate(
        "examTemplateId",
      );
    } else {
...
examSessionRouter.put(
  ...
  async (req, res) => {
    try {
      ...
      session.whitelist = [
        ...new Set([...manualWhitelist, ...classroomEmails]),
      ];
